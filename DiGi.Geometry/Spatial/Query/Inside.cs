...
namespace DiGi.Geometry.Spatial
{
    public static partial class Query
    {
        public static bool Inside<T>(IPlanar<T> planar, Point3D point3D, double tolerance = DiGi.Core.Constans.Tolerance.Distance) where T : IPolygonal2D
        {
            ...
            return Inside(planar, new Point3D[] { point3D }, tolerance);
        }

        public static bool Inside<T>(IPlanar<T> planar, IEnumerable<Point3D> point3Ds, double tolerance = DiGi.Core.Constans.Tolerance.Distance) where T : IPolygonal2D
        {
